...
/**
 * @brief Double precision discrete-time, lattice-based random walk with a 
 *        genealised crowding function. 
 * 
 ...
 * 
 * @todo IMPORTANT! The Neighbourhood function for motility should be distinct 
 * from the neighbourhood function of proliferation. 
 */
int
            ...
            for (int i=0;i<X_tot_prev;i++)
            {
                ...
                else /* handle over crowding*/
                {
                    ...
                    if (u <= fabs(fX))
                    {
                        /*remove the cell*/
                        X[Xind[j]] = 0.0;
                        /*remove fromm index list*/
                        for (int k=j;k<(X_tot-1);k++)
                        {
                            Xind[k] = Xind[k+1];
                        }
                        X_tot--;
