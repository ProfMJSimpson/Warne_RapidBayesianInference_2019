...
/* ABC SMC inference on the discrete model using 
...
main(int argc, char ** argv)
{
    ...
    if (argc  < (6 + 2*m))
    {
        fprintf(stderr,"Usage: %s filename N esp T I [sl1,sl2,...slm,su1,su2,sum] \n",argv[0]);
        exit(1);
    }
    else
    {
        ...
        {
            abc_p.support[k] = (double)atof(argv[6+k]);
        }
        abc_p.sim = NULL;
        abc_p.rho = &rho;
        abc_p.p = &prior;
    ...
    /*generate posterior samples using ABC-SMC and the discrete model*/
    start_t = clock();
