...
/* ABC SMC inference on the Fisher-KPP model using cell density profiles.*/
...
kernel(unsigned int dim,double *theta, double *theta_prop, double *L)
{
    ...
    while (tf)
    {
        /* generate MV Gaussian proposal*/
        durngmvns(dim,theta,L,Z,theta_prop);
        tf = 0;
        /* reject if outside prior support*/
        for (int i=0;i<dim;i++)
        {
...
{
    double D_0;
    D_0 = theta[0]; /* diffusivity */
    
    /* for Fisher-KPP model, diffusivity is linear */
    for (int i=0;i<n;i++)
    {
        Dc[i] = D_0;
    }
...
f(double *c, int n, double *theta, double *fc)
{
    double lambda;
    double K;
    lambda = theta[1]; /* proliferation rate*/
    K = theta[2]; /* carrying capacity density */
    
    /* for Fisher-KPP, logistic growth is used*/
    for (int i=0;i<n;i++)
    {
...
simulate(void *sim, double *theta, Dataset *data_s)
{
    ...
    /*solve using BTCS with fixed-point iterations and adaptive timesteps*/
    dbtcsfpecs(L,T,nt,Ti,theta,C0_f,&D,&f,Nx,Nt,1e-6,1e-6,30,C_s_f);
...
main(int argc, char ** argv)
{
    ...
    else
    {
        ...
        /* @todo sequence is geometric, but could also consider adaptive*/
    ...
    /* generate posterior samples using ABC-SMC and continuum model*/
    start_t = clock();
    ...
    /*write output*/
    fprintf(stdout,"\"Particle\",\"D\",\"lambda\",\"K\",\"weights\",\"SEC\",\"NACC\",\"NSIMS\"\n");
