...
/* ABC adaptive SMC inference on the hexagonal lattic model with a generalised 
 * proliferation function of Jin et al. (2016) Phys. Biol. using cell density 
 * profiles. Specifically looking at prliferation with an Allee effect.
 */
...
f(double Cavg, double *params)
{
    ...
    double A = params[1];
    double K = params[2];
    return Pp*(1.0 - Cavg/K)*((Cavg + A)/K); /* leads to Weak Allee effect model*/
...
main(int argc, char ** argv)
{
    ...
    if (argc  < (8 + 2*m))
    {
        fprintf(stderr,"Usage: %s filename N esp T Pm I J [sl1,sl2,...slm,su1,su2,sum] \n",argv[0]);
        exit(1);
    }
    else
    {
        ...
        abc_p.k = m; /* lambda, beta and gamma*/
    ...
    /*generate posterior samples using adaptive ABC-SMC*/
    start_t = clock();
